...
def save_detected_piece(frame, frame_count, x, y, w, h):
    """Salva a parte da tela que contém a peça detectada em tons de cinza sem texto"""
    timestamp = time.strftime("%Y%m%d-%H%M%S")

    # Recorta a peça detectada
    detected_piece = frame[y:y + h, x:x + w]

    # Converte a imagem para tons de cinza
    detected_piece_gray = cv2.cvtColor(detected_piece, cv2.COLOR_BGR2GRAY)

    # Salva a imagem em tons de cinza
    image_path = os.path.join(DETECTED_PIECES_PATH, f"piece_{frame_count}_{timestamp}.png")
    cv2.imwrite(image_path, detected_piece_gray)
...
while True:
    ...
    for cnt in contours:
        ...
        # Desenha o retângulo ao redor da peça (não será salvo na imagem)
        cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)

        # Salva a imagem da peça detectada em tons de cinza sem o texto
